...
public class GameManager : MonoBehaviour
{
    ...
    // ========== Луч ==========
    [Header("Ray Settings")]
    public float rayDownAngle = 20f;    // угол вниз от направления контроллера
    ...
    // Сохранённые исходные данные для сброса
    ...
    // Позиции элементов UI для анимации сдвига
    ...
    void Start()
    {
        // Сохраняем оригинальное состояние ландшафта
    ...
    void OnDestroy()
    {
        // Полный сброс при остановке сцены
        ResetTerrainToOriginal();
    }

    void ResetTerrainToOriginal()
    {
        if (currentTerrainData == null || originalHeights == null)
            return;
    ...
    void UpdateLaser(bool gripPressed)
    {
        ...
        // Направление: вперёд контроллера, наклонённое вниз на rayDownAngle градусов
    ...
    void ExecuteAction(Transform controllerTransform)
    {
        ...
        // Та же логика направления, что и у лазера
    ...
    void PourWater(Vector3 center)
    {
        ...
        float maxSearchRadius = 50f;   // максимальный радиус поиска края
        float step = 0.05f;           // шаг сканирования для точного контура

        // Массивы для хранения максимумов по каждому лучу
        float[] maxHeights = new float[rays];
        // Список для точек контура
        List<Vector3> contourPoints = new List<Vector3>();

        // Первый проход: ищем максимальную высоту вдоль каждого луча
        for (int i = 0; i < rays; i++)
        {
            ...
            while (dist < maxSearchRadius)
            {
                ...
                float h = terrain.SampleHeight(testPoint);
                if (h > maxH) maxH = h;
                dist += step * 4; // прореживаем для ускорения поиска максимума (шаг 0.2 м)
            }
            maxHeights[i] = maxH;
        }

        // Уровень воды = минимум из максимумов (точка перелива)
        float waterLevel = Mathf.Min(maxHeights);

        // Если перепад высот слишком мал, это не яма
        ...
        for (int i = 0; i < rays; i++)
        {
            ...
            float prevDist = 0f;
            float prevHeight = bottomHeight;
            bool found = false;

            // Идём от центра наружу, пока высота не превысит waterLevel
            float dist = 0f;
            while (dist < maxSearchRadius)
            {
                Vector3 testPoint = center + dir * dist;
                ...
                if (h >= waterLevel)
                {
                    // Пересечение между prevDist и dist
                    float t = 0f;
            ...
            // Если не нашли (край слишком далеко или вода переливается за горизонт), добавляем точку на максимальной дальности
            if (!found)
            {
        ...
        if (contourPoints.Count < 3) return; // недостаточно точек для меша

        Vector3 waterCenter = new Vector3(center.x, waterLevel, center.z);

        // Удаляем старую воду поблизости
        RemoveOldWater(waterCenter, 5.0f);
    ...
    void CreateUI()
    {
        ...
        // Кисть (видна всегда, кроме Воды)
        brushSizeText = CreateText("BrushLabel", uiRoot.transform, "Кисть: 2.5", 20, TextAlignmentOptions.Left);
        brushSizeText.rectTransform.anchorMin = new Vector2(0, 1);
        brushSizeText.rectTransform.anchorMax = new Vector2(0, 1);
        ...
        strengthText.rectTransform.anchorMin = new Vector2(0, 1);
        strengthText.rectTransform.anchorMax = new Vector2(0, 1);
        ...
        paintStrengthText.rectTransform.anchorMin = new Vector2(0, 1);
        paintStrengthText.rectTransform.anchorMax = new Vector2(0, 1);
        ...
        yPos -= sliderHeight + 65f;

        // ---------- Выбор слоя текстуры (Paint) ----------
        float arrowButtonWidth = 120f;
        float arrowButtonHeight = 55f;
        ...
        // ---------- Кнопки режимов ----------
        float modeButtonWidth = 140f;
        float modeButtonHeight = 55f;
    ...
    Slider CreateSlider(string name, Transform parent, Vector2 anchoredPos, Vector2 sizeDelta, float minVal, float maxVal, float defaultVal)
    {
        ...
        fillAreaRect.sizeDelta = new Vector2(-30, 0);
        fillAreaRect.anchoredPosition = new Vector2(15, 0);
        ...
        handleRect.sizeDelta = new Vector2(12, 24);
    ...
    void UpdateUI()
    {
        ...
        // Сдвиг ползунка краски и слоя вверх, когда режим Paint
        if (paintMode)
        {
        ...
        {
            // Возврат на исходные позиции (если были сдвинуты)
